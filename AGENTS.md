...
> Instructions coding agents must follow in every session.
...
- `.agents/commands` is the canonical shared commands directory. `.claude/commands` symlinks to it.
- `.agents/commands/tm` contains the Task Master command docs.
...
- The shared file tree hook will auto-update docs/infra/file-tree.md when `.agents/session-changed` exists
...
1. Delete `.agents/session-changed` to cancel pending stop hooks
2. Do NOT run the end-of-session wrap-up
3. Report to user: "Hit 3-retry limit on [operation]. Need your help to proceed."
...
- The shared file tree hook will auto-update docs/infra/file-tree.md
- Run the end-of-session wrap-up in `.agents/skills/wrap-up/SKILL.md` to review docs and clear `.agents/session-changed`
